<table width=750><tr><td width=500><h2>{phraseBienvenu}</h2></td></tr></table>
...
<input type='hidden' name='idUtilisateurModif' value='{idUtilisateurModif}' />
    ...
    <tr><td valign=middle>Votre avatar</td><td valign=middle ><div style='float:left;padding-top:50px;'><INPUT type=hidden name=MAX_FILE_SIZE  VALUE=8388608><input type='file' name='fichierAvatar' accept="image/jpeg" value=''><br><?_("Taille de fichier max : 5 Mo")?><br><?_("Cochez pour supprimer")?>&nbsp;<input type='checkbox' name='supprFichierAvatar' value='1'></div>{detailUtilisateur.imageAvatar}</td><td>{detailUtilisateur.imageAvatar-error}</td></tr>
