...
class archiAccueil extends config
{
    ...
    function afficheAccueil($params=array())
    {
        ...
        switch($modeAffichage)
        {
            ...
            case 'monArchi':
                ...
                $arrayPaginationEvenements=$paginationEvenements->pagination(array(
                ...
                $res = $this->connexionBdd->requete($req);
                $nbEnregistrementTotaux=mysql_num_rows($res);
                $arrayPaginationAdresses=$paginationAdresses->pagination(array(
                ...
                $res = $this->connexionBdd->requete($req);
                $nbEnregistrementTotaux=mysql_num_rows($res);
                $arrayPaginationCommentaires=$paginationCommentaires->pagination(array(
                                        'nomParamPageCourante'=>'archiMonArchiAdressesPage', 
                                        'nbEnregistrementsParPage'=>$nbEnregistrementsParPage, 
                                        'nbEnregistrementsTotaux'=>$nbEnregistrementTotaux, 
                                        'typeLiens'=>'noformulaire'
    ...
    // affiche les encarts a partir du tableau contenant les differentes informations necessaire ,  adresses par encart ,  indice des adresses comportant des images ,  et idHistoriqueImage + dateUpload de l'image
    public function getEncarts($params = array())
    {
        ...
        if (count($params['demolitions'])>0)
        {
            ...
            $tDemolitions->assign_vars(array(
                            'photoAdresse1'=>"<table border='' width=200 height=150 style='border:1px #000000 solid;margin-right:2px;float:left;'><tr><td style='background-image:url(".$this->urlImages."imageDefautArchiv2.jpg);padding-left:50px;padding-top:70px;font-size:11px;'>Pas de démolitions</td></tr></table>", 
        ...
        if (count($params['constructions'])>0)
        {
            ...
            $tTravaux->assign_vars(array(
                            'photoAdresse1'=>"<table border='' width=200 height=150 style='border:1px #000000 solid;margin-right:2px;float:left;'><tr><td style='background-image:url(".$this->urlImages."imageDefautArchiv2.jpg);padding-left:50px;padding-top:70px;font-size:11px;'>Pas de travaux</td></tr></table>", 
        ...
        if (count($params['culture'])>0)
        {
            ...
            $tCulturel->assign_vars(array(
                            'photoAdresse1'=>"<table border='' style='border:1px #000000 solid;margin-right:2px;float:left;'><tr><td style='background-image:url(".$this->urlImages."imageDefautArchiv2.jpg);padding-left:40px;padding-top:70px;font-size:11px;'>Pas d'évènements culturels</td></tr></table>", 
        ...
        if (count($params['dernieresAdresses'])>0)
        {
            ...
            $tDernieresAdresses->assign_vars(array(
                            'photoAdresse1'=>"<table  border='' width=200 height=150 style='border:1px #000000 solid;margin-right:2px;float:left;'><tr><td style='background-image:url(".$this->urlImages."imageDefautArchiv2.jpg);padding-left:40px;padding-top:70px;font-size:11px;'>Pas de nouvelles adresses</td></tr></table>", 
        ...
        foreach ($params['dernieresAdresses'] as $indice => $value)
        {
            ...
            if (isset($params['indiceEvenementsPremierePositions']['dernieresAdresses']) && $indice == $params['indiceEvenementsPremierePositions']['dernieresAdresses'])
            {
                ...
                if ($params['imagesEvenementsPremieresPositions']['dernieresAdresses']['idHistoriqueImage']!='')
                {
                    ...
                    $tDernieresAdresses->assign_vars(array(
                                'photoAdresse1'=>"<table border='' width=200 height=150 style='border:1px #000000 solid;margin-right:2px;float:left;'><tr><td style='background-image:url(".$this->urlImages."imageDefautArchiv2.jpg);padding-left:70px;padding-top:70px;font-size:11px;'>Pas de photo</td></tr></table>", 
                                'descriptionAdresse1'=>"<div><a href='".$this->creerUrl('', '', array('archiAffichage'=>'adresseDetail', "archiIdAdresse"=>$value['idAdresse'], "archiIdEvenementGroupeAdresse"=>$value['idEvenementGroupeAdresse']))."' style='font-size:12px;'>".date('d/m/Y', strtotime($value['dateCreationAdresse'])).' '.$intituleAdresse."</a><br>".($string->sansBalises(stripslashes($string->coupureTexte($value['description'],  20))))."</div>"
                                ));
    ...
    // recupere la listes des architectes les plus productifs classée
    ...
    // recupere la liste des rues les plus completes (ou il y a le plus d'adresses dans la rue) classée
    ...
    public function gestionSondage($params = array())
    {
        ...
        if (isset($params['modeAffichage']) && $params['modeAffichage']=='resultatAccueil')
        {
            ...
            $c = new calqueObject();
            $t->assign_vars(array('jsScroll'=>$c->getJsScrollDivWithPage(array(
    ...
    public function getDernieresActualites($params = array())
    {
        ...
        $sqlFields = "idActualite,  titre,  date,  photoIllustration,  texte,  urlFichier";
        ...
        $req = "SELECT $sqlFields FROM actualites WHERE 1=1 $sqlWhere ORDER BY date DESC $sqlLimit";
        $res = $this->connexionBdd->requete($req);
        
        $i=0;
        if (!isset($params['returnAsMysqlRes']) || $params['returnAsMysqlRes']!=true) {
            $retour = array();
            while ($fetch = mysql_fetch_assoc($res)) {
                $retour[$i] = $fetch;
                $i++;
