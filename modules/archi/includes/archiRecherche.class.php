...
class archiRecherche extends config {
	...
	*/
	public function search($criteres = array()){
		$html = '';
		$tabForm = array();
		$resAvAdresse="";
		$formulaire = new formGenerator();
	...
	*/
	public function advancedSearch($criteres=array()){
		...
		*/
		...
		$sqlWhere = $this->buildWhereClause($criteres); // Generating the where clause with the criterias
		$idHistoriqueEvenementArray = $this->getIdHistoAdresses($sqlWhere,$params); //Execute the fulltext research with the Where clause generated by
	...
	private function getIdHistoAdresses($sqlWhere = '',$params =array()){
		...
		if(isset($params['motcle']) && $params['motcle']!=''){
			$request = "SELECT idHistoriqueAdresse, idEvenementGA, nomRue,nomSousQuartier,nomQuartier,nomVille,nomPays,prefixeRue,description,titre,nomPersonne, prenomPersonne, numeroAdresse,concat1,concat2,concat3 ,
				...
				1 * (MATCH (titre) AGAINST ('".$params['motcle']."' IN BOOLEAN MODE)) +
				1000 * (MATCH (concat1) AGAINST ('".$params['motcle']."' IN BOOLEAN MODE)) +
				100 * (MATCH (concat2) AGAINST ('".$params['motcle']."' IN BOOLEAN MODE)) +
				100 * (MATCH (concat3) AGAINST ('".$params['motcle']."' IN BOOLEAN MODE))
	...
	public function searchByRue($rue = ""){
		if(!empty($rue)){
			$sqlWhere = $this->buildWhereClause(array('rue'=>$rue)); // Generating the where clause with the criterias
			$idHistoriqueEvenementArray = $this->getIdHistoAdresses($sqlWhere,$params); //Execute the fulltext research with the Where clause generated by
			$nbResult = count($this->getIdHistoAdresses($sqlWhere,array('motcle' => $motcle))); //Count the nb of result
			$a = new archiAdresse();
			$html .=$a->displayList($idHistoriqueEvenementArray,$nbResult);
			...
			$html.="Aucune addresse à afficher.";
		}
