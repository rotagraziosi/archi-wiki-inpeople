...
if (isset($_GET['archiAffichage'])) {
    ...
    if (in_array($archiAffichage, $modesAffichagesAvecAuthentification) 
        ...
        case 'listeAdressesFromRue':
        	$r=new archiRecherche();
        	$html = $r->searchByRue($_GET['recherche_rue']);
        	
        	
        	/*
        	$sqlWhere = $this->buildWhereClause(array('rue'=>$_GET['recherche_rue'])); // Generating the where clause with the criterias
        	...
        	/*
        	$r = new archiRecherche();
        	$tab = $r->advancedSearch(array('recherche_rue'=>$_GET['recherche_rue']));
            /*$a = new archiAdresse();
            $tab = $a->displayList(
