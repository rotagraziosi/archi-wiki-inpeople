...
// 
// Version: v2.2
// Copyright (c) 2005-2007 - Sylvain BAUDOIN
...
function Calendar($params) {
	...
	if (! function_exists("calendar_calculate_URL")) {
		function calendar_calculate_URL($URL, $URL_PARAMETER, $date, $PRESERVE_URL, $USE_SESSION, $isInJSPopup=false) {
			...
			// parametre ajouté par laurent pour la gestion du retour en javascript
			if($isInJSPopup)
			{
				$new_URL = "javascript:parent.document.getElementById(parent.document.getElementById('paramChampAppelantDate').value).value='".$date."';parent.document.getElementById('calqueDate').style.display='none';";
			}
	...
	for ($row = 1; $row <= $row_number; $row++) {
		...
		for ($column = 1; $column <= 7; $column++) {
			...
			if ($DATE_URL != "") 
			{
				...
				$table_cell .= "<a href=\"#\" onclick=\"".calendar_calculate_URL($DATE_URL, $URL_PARAMETER, gmdate($URL_DAY_DATE_FORMAT, $current_day), true, $USE_SESSION,true)."\" class=\"".$CSS_class."\">".$day."</a>";
			} 
			else 
			{
				// Days not in the current month with CSS class "other_month"
