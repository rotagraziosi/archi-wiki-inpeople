...
class checkBrowserObject extends config
{
	...
	// fonction javascript permettant de connaitre la version de java et si pas assez recente, installation automatique de la derniere version
	// la premiere partie du script provient du fichier => <script src=\"http://java.com/js/deployJava.js\"></script>
	public function getJsCheckJava($params = array())
	{
	
		if(isset($params['jsErrorMsgContainerId']) && $params['jsErrorMsgContainerId']!='')
		{
			$messageContainer = $params['jsErrorMsgContainerId'];
		}
		
		
		$ifNoJavaAction = "deployJava.installLatestJRE();";
		if(isset($params['ifNoJavaAction']) && $params['ifNoJavaAction']!='')
		{
			$ifNoJavaAction = $params['ifNoJavaAction'];
		}
		
		$ifNoGoodJavaVersion ="deployJava.installJRE('1.6.0_10+');";
		if(isset($params['ifNoGoodJavaVersion']) && $params['ifNoGoodJavaVersion']!='')
		{
			$ifNoGoodJavaVersion = $params['ifNoGoodJavaVersion'];
		}
...
deployJava.locale=loc;}}}};deployJava.do_initialize();
		...
		if (deployJava.isPluginInstalled()) {
			//alert('Plugin is installed');
			if (!deployJava.versionCheck('1.6.0_10+')) {
				document.getElementById('$messageContainer').innerHTML=\"<font color='red'>Attention, votre version de java est peut être trop ancienne. Téléchargez la dernière version en <a href='http://www.java.com'>cliquant ici</a>. Puis rechargez la page.</font>\";
			...
			document.getElementById('$messageContainer').innerHTML=\"<font color='red'>Attention le plugin java n'est pas installé téléchargez la dernière version en <a href='http://www.java.com'>cliquant ici</a>. Puis rechargez la page.</font>\";
